...
def full_name_gen_uncorrupted_record(master_record, record_to_modify={}):
    full_name = (
        master_record["given_nameLabel"]
        + " "
        + master_record["family_nameLabel"]
    )
    record_to_modify["full_name"] = full_name.lower()
    return record_to_modify

    
def name_gen_uncorrupted_record(
    master_record, record_to_modify, input_colname, output_colname
):
    record_to_modify[output_colname] = master_record[input_colname].lower()
    return record_to_modify
...
def name_typo(
    ...
    name = record_to_modify[input_colname]

    querty_corruptor = CorruptValueQuerty(
        position_function=position_mod_uniform, row_prob=0.5, col_prob=0.5
    )

    record_to_modify[output_colname] = querty_corruptor.corrupt_value(name)
